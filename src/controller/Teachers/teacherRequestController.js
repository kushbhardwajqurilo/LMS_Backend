...
exports.TerminateTeacher = async (req, res) => {
  try {
    ...
    const teacher = await TeacherProfileModel.findOne({ userId: teacherId });
    if (teacher.isExit === true) {
      return res
        .status(200)
        .json({ success: false, message: "Teacher Already Terminate" });
    }
    const exitTeacher = await TeacherProfileModel.findOneAndUpdate(
      { userId: teacherId },
      {
        $set: { isExit: true, exitedAt: new Date() },
      }
    );
    if (!exitTeacher) {
      return res
        .status(400)
        .json({ success: false, message: "Unable to Terminate Teacher" });
    }
    return res
      .status(200)
      .json({ success: true, message: "Teacher Terminate Successfull" });
  } catch (error) {
    console.log(error);
    res.json({
      success: false,
