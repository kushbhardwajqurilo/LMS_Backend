...
exports.createCourseCheckout = async (req, res) => {
  try {
    ...
    if (!amount || !courseId) {
      return res.status(400).json({
        success: false,
        message: "Missing required fields: amount or courseId",
      });
    }
    ...
    // ✅ Add course to enrolledCourses manually
    ...
    // ✅ Populate the newly enrolled course details
    const updatedStudent = await StudentProfileModel.findOne({ userId: studentId })
      .populate("enrolledCourses.courseId");

    const enrolledCourseData = updatedStudent.enrolledCourses.find(
    ...
    return res.status(200).json({
      ...
      data: {
        studentId,
        courseId: enrolledCourseData.courseId._id,
        amount,
        enrolledCourse: enrolledCourseData, // full course data populated
      },
    });
...
exports.createBookingCheckout = async (req, res) => {
  try {
    ...
    // Format callback URLs (optional use)
    const formattedReturnUrl =
      returnUrl || `${req.headers.origin}/student-dashboard/booking/payment-success?session_id=${paymentId}`;
    const formattedCancelUrl =
      cancelUrl || `${req.headers.origin}/cancel?session_id=${paymentId}`;
    const formattedNotifyUrl =
    ...
    // Save payment record (can be used by any gateway)
    await PaymentModel.create({
      userId: studentId,
      teacherId,
      amount: parseFloat(formattedAmount),
      ...
      status: "pending",
      sessionId: paymentId,
      paymentMethod: "free", // ❗ change this to 'stripe', 'paypal', etc. if needed
