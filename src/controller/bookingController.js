...
exports.createBooking = async (req, res) => {
  try {
    ...
    // ✅ Validate required fields (fixed to allow amount = 0)
    ...
    // 🟩 Fetch course/subject name (optional)
    let courseName = "Course";
    try {
      const subject = await CourseSubCategoryModel.findById(subjectId);
      if (subject) courseName = subject.name;
    } catch (err) {
      console.warn("Subject fetch failed:", err.message);
    }
    ...
    console.log("se Booking", newBookingObj);
    const newBooking = await BookingModel.create(newBookingObj);
    ...
    if (studentProfile && Array.isArray(studentProfile.tutionBookings)) {
      studentProfile.tutionBookings.push(newBooking._id);
      await studentProfile.save();
    } else {
      console.warn(
        `Student profile not found or invalid for userId: ${studentId}`,
      );
    }

    if (teacherProfile && Array.isArray(teacherProfile.tutionBookings)) {
      teacherProfile.tutionBookings.push(newBooking._id);
      await teacherProfile.save();
    } else {
      console.warn(
        `Teacher profile not found or invalid for userId: ${teacherId}`,
      );
    }
    ...
    if (student && teacher) {
      await emailService.sendBookingScheduled({
        ...
        startTime: formattedStartTime,
        endTime: formattedEndTime,
        courseName: `${courseName} (${sessionDuration} Minutes)`,
        amount: newBooking.amount,
      });
    } else {
      console.warn("Student or teacher not found; skipping email.");
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...newBooking.toObject(),
        sessionTitle: `${courseName} (${sessionDuration} Minutes)`,
        teacherName: teacher
...
exports.getBookingsForTutor = async (req, res) => {
  try {
    let { startDate, endDate, teacherId } = req.query;
...
exports.confirmBooking = async (req, res) => {
  try {
    ...
    const formattedStartTime = moment(booking.sessionStartTime).format(
      "h:mm A",
    );
...
exports.editBookingMeetingInfo = async (req, res) => {
  try {
    ...
    if (
      oldMeetingPlatform !== booking.meetingPlatform ||
      oldMeetingLink !== booking.meetingLink
    ) {
      // Get student and teacher details for email
      const student = await UserModel.findById(booking.studentId);
      const teacher = await UserModel.findById(booking.teacherId);
      ...
      let courseName = "Course";
      try {
        const subject = await CourseSubCategoryModel.findById(
          booking.subjectId,
        );
        if (subject) {
          courseName = subject.name;
  ...
  try {
    ...
    if (booking.hasBeenRescheduled) {
      return res.status(400).json({
        success: false,
        message:
          "This booking has already been rescheduled once and cannot be rescheduled again",
      });
    }
  ...
  try {
    ...
    if (!isAdmin) {
      ...
      const isValidResponder =
        (requestedById === booking.studentId.toString() &&
          responderId === booking.teacherId.toString()) ||
        (requestedById === booking.teacherId.toString() &&
          responderId === booking.studentId.toString());

      if (!isValidResponder) {
        return res.status(403).json({
          success: false,
          message:
            "Unauthorized. Only the other party can respond to the reschedule request",
        });
      }
      ...
      if (action === "accept") {
        ...
        const newEndTime = new Date(
          newTimeObj.getTime() + sessionDurationMinutes * 60 * 1000,
        );
    ...
    try {
      if (!isAdmin) {
        ...
        if (action === "accept") {
          ...
          await emailService.sendRescheduleRejection({
            ...
            requestedBy:
              booking.rescheduleRequest.rescheduleBy.toString() ===
      ...
      console.error(
        `Error sending ${action === "accept" ? "confirmation" : "rejection"} emails:`,
        emailError,
      );
    }
