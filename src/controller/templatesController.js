...
const signup = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: ` sent successfully to ${teacherEmail}`,
    });
  } catch (error) {
...
const teacherRequest = async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: settings.smtpUsername || process.env.EMAIL_USER,
      to: email,
      subject: status === "approved" ? "Teacher Request" : "Teacher Request",
      html: emailTemplate,
    };
...
const sendBookingConfirmation = async (req, res) => {
  try {
    ...
    // console.log(
    //   "Received email request for student:",
    //   studentEmail,
    //   "and teacher:",
    //   teacherEmail,
    // );
    ...
    return res.status(200).json({
      success: true,
      message:
        "Booking confirmation emails sent successfully to both student and teacher",
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to send booking confirmation" });
  }
};
