    ...
    const courseEarnings = await OrderModel.aggregate([
      ...
      },
      {
        ...
        $project: {
          ...
          enrollments: 1,
          totalAmount: 1,
          commission: { $multiply: ["$totalAmount", 0.4] }, // 40% commission
          earnings: { $multiply: ["$totalAmount", 0.6] }, // 60% earnings
        },
      },
      {
    ...
    return res.json({
      ...
      message:
        courseEarnings.length > 0
          ? "Course Purchases Found Succesfully"
          : "No Course Purhcase Found",
      data: courseEarnings,
      totalPages: courseEarnings[0]?.total || 0 / parseInt(limit),
    });
  } catch (error) {
...
exports.getTutoringEarnings = async (req, res) => {
  try {
    ...
    return res.json({
      ...
      message:
        bookings.length > 0
          ? "Tutoring Sessions Found Succesfully"
          : "No Data found",
      data: bookings,
      totalPages: bookings[0]?.total || 0 / parseInt(limit),
    });
  } catch (error) {
    console.log(error);
    res
