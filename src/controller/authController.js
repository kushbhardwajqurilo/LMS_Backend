...
async function sendVerificationEmail(user) {
  try {
    ...
    const templateData = {
      ...
      message:
        "Please click the button below to verify your email address and activate your account. If you did not create an account, please ignore this email.",
    };
...
exports.userLogin = async (req, res) => {
  try {
    const data = req.body;
    console.log("len", req.body.password.length);
    // Find user by email
    const user = await UserModel.findOne({ email: data.email });
    ...
    // Set token as a cookie (optional if not using cookies for auth)
    res.cookie("token", token, {
      sameSite: "none",
      secure: process.env.COOKIE_SECURE === "true", // from .env
      httpOnly: true,
    ...
    return res.status(200).json({
      status: "success",
      message: "Login successfully",
      token, // ✅ Token now included in response
      data: {
        _id: user._id,
        email: user.email,
        role: user.role,
...
exports.validateToken = async (req, res) => {
  try {
    ...
    const { userStatus, role, email, firstName, lastName, _id, profilePhoto } =
      user;
    const userData = {
      _id,
