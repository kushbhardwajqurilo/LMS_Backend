...
const paymentSchema = new mongoose.Schema({
  ...
  status: { 
    type: String, 
    enum: ["pending", "succeeded", "refunded", "failed", "cancelled", "expired"], 
    default: "pending" 
  },
  sessionId: { type: String, required: true, unique: true },
  ...
  paymentMethod: { 
    type: String, 
    enum: ["paypal", "stripe", "wallet", "bank_transfer", "free"],  // ✅ FIXED: added "free" correctly
    required: true 
  },
  refundStatus: { type: String, enum: ["not_initiated", "processing", "completed"] },
  refundAmount: { type: Number, default: 0 },
  paymentStatus: { 
    type: String, 
    enum: ["paid", "unpaid", "failed", "succeeded"],  // ✅ FIXED: added "succeeded"
    default: "unpaid" 
  },
  metadata: { type: Object, required: true }
}, { timestamps: true });
